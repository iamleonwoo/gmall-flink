...
//        env.setStateBackend(new FsStateBackend("hdfs://hadoop102:8020/flink/ck"));
        ...
        //TODO 3.统一数据格式
        //3.1 处理UV数据
        SingleOutputStreamOperator<VisitorStats> visitStatsWithUvDS = uvDS.map(value -> {
            //1.将数据转换为JSONObject
        ...
        //3.2 处理UJ数据
        SingleOutputStreamOperator<VisitorStats> visitStatsWithUjDS = ujDS.map(value -> {
            //1.将数据转换为JSONObject
            JSONObject jsonObject = JSONObject.parseObject(value);
            //2.提取公共字段
            JSONObject common = jsonObject.getJSONObject("common");
        ...
        //3.3 处理PV数据
        SingleOutputStreamOperator<VisitorStats> visitStatsWithPvDS = pageViewDS.map(value -> {
            //1.将数据转换为JSONObject
            JSONObject jsonObject = JSONObject.parseObject(value);
            //2.提取公共字段
            JSONObject common = jsonObject.getJSONObject("common");
        ...
        visitorStatsDS.addSink(ClickHouseUtil.getSink("insert into visitor_stats_2024 values(?,?,?,?,?,?,?,?,?,?,?,?)"));
